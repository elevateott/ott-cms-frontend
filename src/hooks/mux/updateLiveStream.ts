...
export const updateLiveStream: CollectionBeforeChangeHook = async ({
  ...
  try {
    ...
    if (data.isRecordingEnabled !== undefined && 
      ...
      updateData.recording = data.isRecordingEnabled
      muxChangesDetected = true
    }
    
    // Check for reconnect window changes
    if (data.reconnectWindow !== undefined && 
    ...
    if (muxChangesDetected) {
      ...
      try {
        ...
        throw new Error(`Failed to update Mux live stream: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
    
    // Check for simulcast target changes
    if (data.simulcastTargets && 
