...
export async function POST(req: Request) {
  try {
    ...
    const url = stripe.oauth.authorizeUrl({
      client_id: testMode ? process.env.STRIPE_TEST_CLIENT_ID : process.env.STRIPE_LIVE_CLIENT_ID,
    ...
    return NextResponse.json({ error: 'Failed to generate Stripe OAuth URL' }, { status: 500 })
  }
}
