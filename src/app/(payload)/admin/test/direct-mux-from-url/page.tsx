...
export default function DirectMuxFromUrlPage() {
  const [selectedFile, setSelectedFile] = useState<{ name: string; url: string } | null>(null)
  const [isLoading, setIsLoading] = useState(false)
  const [uploadStatus, setUploadStatus] = useState<'idle' | 'creating' | 'success' | 'error'>(
    'idle',
  )
  ...
  const handleFileSelected = (file: { name: string; url: string }) => {
    ...
    // Log file details
    clientLogger.info('File selected', 'DirectMuxFromUrl', {
      name: file.name,
  ...
  return (
    <div className="container py-8">
      <h1 className="text-2xl font-bold mb-6">Direct Mux From URL Test</h1>
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Testing creating a Mux asset directly from a Dropbox URL
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="pt-4">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      setIsLoading(true)
                      clientLogger.info('Dropbox button clicked', 'DirectMuxFromUrl')
                      ...
                      clientLogger.info('Creating Mux asset from URL', 'DirectMuxFromUrl', {
                      ...
                      if (!response.ok) {
                        ...
                        try {
                          ...
                          clientLogger.error('Error parsing error response', 'DirectMuxFromUrl', {
                      ...
                      try {
                        ...
                        clientLogger.error('Error parsing success response', 'DirectMuxFromUrl')
                        throw new Error('Failed to parse response from server')
                      }

                      clientLogger.info('Mux asset created successfully', 'DirectMuxFromUrl', {
                      ...
                      if (errorMsg === 'Dropbox selection cancelled') {
                        ...
                        clientLogger.error('Error in Dropbox upload process', 'DirectMuxFromUrl', {
                          error: errorMsg,
          ...
          <CardFooter>
            <div className="text-sm text-gray-500">
              This test page demonstrates creating a Mux asset directly from a Dropbox URL. This is
              more efficient than downloading the file and then uploading it to Mux.
            </div>
          </CardFooter>
        </Card>
      </div>
