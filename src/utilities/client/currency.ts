...
/**
 * Client-safe utility functions for currency handling
 * This version doesn't depend on server-only code
 */

// Default currency settings for client-side use
...
export function detectUserCurrency(): string {
  try {
    ...
    if (detectedCurrency && DEFAULT_CURRENCY_SETTINGS.supportedCurrencies.includes(detectedCurrency)) {
      return detectedCurrency
    }
    
    // Fallback to default currency
    return DEFAULT_CURRENCY_SETTINGS.defaultCurrency
  } catch (error) {
...
export function getUserCurrency(): string {
  try {
    ...
    if (storedCurrency && DEFAULT_CURRENCY_SETTINGS.supportedCurrencies.includes(storedCurrency)) {
