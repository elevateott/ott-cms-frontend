...
export const ExternalHlsPreviewPlayer: React.FC = () => {
      ...
      if (streamStatus !== 'offline') {
        ...
        if (streamStatus === 'live') {
          toast({
            ...
            description: 'The external stream is currently unavailable. Attempting to reconnect...',
            variant: 'destructive',
            duration: 4000,
          })
        }
  ...
  // Active retry when stream is offline
  useEffect(() => {
    if (streamStatus === 'offline' && isValidUrl) {
      logger.info('Setting up active retry for offline stream')

      const retryTimeout = setTimeout(() => {
        const video = videoRef.current
        if (video) {
          logger.info('Actively retrying to load and play the stream')
          ...
          video.play().catch((error) => {
            logger.warn('Autoplay prevented during retry:', error)
          })

          // Update status to loading
          setStreamStatus('loading')
        }
      }, 5000) // Wait 5 seconds before active retry

      return () => clearTimeout(retryTimeout)
    }
  }, [streamStatus, isValidUrl])
    ...
    return (
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>Preview your external HLS stream</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert variant="destructive">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>
              Invalid External HLS URL. The URL must start with http:// or https:// and end with
              .m3u8
            </AlertDescription>
          </Alert>
        </CardContent>
      ...
      <CardContent className="space-y-4">
        ...
        {streamStatus === 'offline' && (
          <Alert variant="destructive">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>
              Stream is currently offline or unavailable. Attempting to reconnect automatically...
            </AlertDescription>
          </Alert>
        )}
