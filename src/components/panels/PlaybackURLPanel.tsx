...
export const PlaybackURLPanel: React.FC = () => {
  ...
  const generateSignedUrl = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/live-events/${document.id}/signed-playback?ttl=${ttl}`)
  ...
  // If there are no playback IDs, show a message
    ...
    return (
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            No playback URLs available yet
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Alert>
    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Alert variant="warning">
                  <AlertDescription className="text-sm">
                    This stream requires a signed token for playback. Generate a temporary signed URL below.
                  </AlertDescription>
                </Alert>
                  ...
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className="flex items-center gap-1 whitespace-nowrap"
                      >
                        {copied['signed'] ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                    ...
                    <div className="text-sm text-gray-500 flex items-center gap-1">
                      ...
                      <span>This URL will expire in {ttl === 60 ? '1 minute' : ttl === 300 ? '5 minutes' : ttl === 900 ? '15 minutes' : '1 hour'}.</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Preview Player (for public streams) */}
          {playbackPolicy === 'public' && ['active', 'idle'].includes(muxStatus) && (
