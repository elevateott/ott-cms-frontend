...
export function DropboxButton({ appKey, onFileSelected, disabled = false }: DropboxButtonProps) {
  const [isLoading, setIsLoading] = useState(false)
  // Initialize as true to ensure the button is enabled immediately
  const [isDropboxLoaded, setIsDropboxLoaded] = useState(true)

  // Load Dropbox SDK on mount
  useEffect(() => {
    if (!appKey) return

    console.log('Loading Dropbox SDK on mount with key:', appKey)

    // Check if script already exists
    const existingScript = document.getElementById('dropboxjs')
    if (existingScript) {
      console.log('Dropbox script already exists')
      // If script exists, assume Dropbox is loaded
    ...
    script.onerror = (e) => {
      ...
      setIsDropboxLoaded(false)
    }

    // Append to document body
    document.body.appendChild(script)
    console.log('Dropbox script appended to document body')

    // Clean up
    return () => {
      const scriptToRemove = document.getElementById('dropboxjs')
      if (scriptToRemove) {
  ...
  const handleDropboxSelect = () => {
    ...
    console.log('Dropbox button clicked', {
      isDropboxLoaded,
      hasDropboxGlobal: typeof window !== 'undefined' && !!window.Dropbox,
      appKey,
    })

    // If Dropbox isn't loaded yet, try to load it
    if (!window.Dropbox) {
      clientLogger.warn('Dropbox SDK not loaded, attempting to load it now', 'DropboxButton')
  ...
  const openDropboxChooser = () => {
    try {
          ...
          if (files && files.length > 0) {
            ...
            if (file && file.name && file.link) {
              downloadFileFromUrl(file.link, file.name)
                ...
                .catch((error) => {
                  clientLogger.error('Error downloading file from Dropbox', 'DropboxButton', {
                    error,
                  })
                  alert('Error downloading file from Dropbox. Please try again.')
                })
            } else {
              clientLogger.error('Invalid file object from Dropbox', 'DropboxButton')
      ...
      setIsLoading(false)
      clientLogger.error('Error initializing Dropbox chooser', 'DropboxButton', { error })
  ...
  const downloadFileFromUrl = async (url: string, fileName: string): Promise<File> => {
    try {
      ...
      clientLogger.error('Error downloading file', 'DropboxButton', { error })
      throw error
    }
  }

  // Always enable the button if we have an app key
  return (
    <Button
    ...
    >
      ...
      {!isDropboxLoaded && !isLoading && (
        <span className="ml-2 text-xs text-gray-500">(initializing...)</span>
      )}
    </Button>
  )
}
