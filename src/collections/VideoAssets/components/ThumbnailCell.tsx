...
const ThumbnailCell = (props: DefaultCellComponentProps) => {
  ...
  const renderContent = () => {
    switch (muxStatus) {
      ...
      case 'processing':
        return (
          <div className="flex items-center justify-center gap-2 w-full h-full text-white text-sm">
            <svg
              aria-hidden="true"
              className="inline w-8 h-8 animate-spin fill-white text-gray-200"
              viewBox="0 0 100 101"
              fill="none"
            ...
            <span className="capitalize">
              {muxStatus == 'uploading' ? 'Uploading...' : 'Transcoding...'}
            </span>
          </div>
        )
  ...
  return (
    <div className={className}>
      <div className="w-[100px] h-[56px] rounded overflow-hidden bg-gray-800">
        {renderContent()}
      </div>
    </div>
  )
