...
const REPORT_FILE = path.resolve(__dirname, 'console-log-replacement-report-v2.md')
...
const patterns = {
  ...
  consoleLog: /console\.log\([\s\S]*?\);?/g,
  consoleError: /console\.error\([\s\S]*?\);?/g,
  consoleWarn: /console\.warn\([\s\S]*?\);?/g,
  consoleInfo: /console\.info\([\s\S]*?\);?/g,
  consoleDebug: /console\.debug\([\s\S]*?\);?/g,
}

// Replacement templates
const serverReplacements = {
  consoleLog: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `logger.info({ context: '${context}' }, ${args})`
  },
  consoleError: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `logger.error({ context: '${context}' }, ${args})`
  },
  consoleWarn: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `logger.warn({ context: '${context}' }, ${args})`
  },
  consoleInfo: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `logger.info({ context: '${context}' }, ${args})`
  },
  consoleDebug: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `logger.debug({ context: '${context}' }, ${args})`
  },
}
...
const clientReplacements = {
  consoleLog: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `clientLogger.info(${args}, '${context}')`
  },
  consoleError: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `clientLogger.error(${args}, '${context}')`
  },
  consoleWarn: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `clientLogger.warn(${args}, '${context}')`
  },
  consoleInfo: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `clientLogger.info(${args}, '${context}')`
  ...
  consoleDebug: (match, context) => {
    const args = match.substring(match.indexOf('(') + 1, match.lastIndexOf(')'))
    return `clientLogger.debug(${args}, '${context}')`
  },
}
  ...
  Object.entries(patterns).forEach(([patternName, pattern]) => {
    ...
    if (matches && matches.length > 0) {
      ...
      matches.forEach((match) => {
        const replacement = replacements[patternName](match, context)
