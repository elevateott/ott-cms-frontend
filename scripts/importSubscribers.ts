    ...
    for (const [index, row] of rows.entries()) {
      try {
        ...
        if (existingSubscribers.docs.length > 0) {
          console.warn(`⚠️ Row ${index + 1}: Subscriber with email ${row.email} already exists, skipping`);
          errorCount++;
          continue;
        }
        ...
        if (row.activePlans) {
          const planIds = row.activePlans.split(',').map(id => id.trim()).filter(Boolean);
          if (planIds.length > 0) {
        ...
        if (row.purchasedPPV) {
          const eventIds = row.purchasedPPV.split(',').map(id => id.trim()).filter(Boolean);
